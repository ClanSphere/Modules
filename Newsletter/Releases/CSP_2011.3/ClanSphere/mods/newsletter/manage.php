...
// $Id: manage.php 4120 2010-06-15 15:29:39Z hajo $
...
$cs_sort[3] = 'users_email DESC';
$cs_sort[4] = 'users_email ASC';
$order = $cs_sort[$sort];
$newsletter_count = cs_sql_count(__FILE__,'newsletter');
$data['head']['ecount'] = cs_sql_count(__FILE__,'newsletter_user');
...
$select = 'nwl.newsletter_id AS newsletter_id, nwl.newsletter_subject AS newsletter_subject, nwl.newsletter_time AS newsletter_time, nwl.users_id AS users_id, usr.users_email AS users_email, usr.users_active AS users_active';
...
for($run=0; $run<$newsletter_loop; $run++) {
  ...
  $data['newsletter'][$run]['user'] = $data['newsletter'][$run]['users_email'];
